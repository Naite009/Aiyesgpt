...
/**
 * Vercel Edge verify endpoint
 * - CORS: permissive for now
 * - Supports: { ping:true }, { mockVerify:true }, and real { image | frames, instruction_step }
 * - Reads env via globalThis.process?.env (Edge safe without Node typings)
 */
...
export default async function handler(req: Request): Promise<Response> {
  ...
  // Health checks (no Gemini)
  if (body?.ping === true) {
    return json({ ok: true, model });
  }
  if (body?.mockVerify === true) {
    const c = typeof body.mockConfidence === "number" ? Math.max(0, Math.min(1, body.mockConfidence)) : 0.9;
  ...
  // Fast timeout so UI never hangs
